# Written-test quizzes — remaining work

Auto-generated. **292 quizzes are already live** on the site. This tracks the rest.


> Note: many entries below are **scanned image tests that DO contain an embedded answer key** (e.g. a letter list on the last page) — those are recoverable via OCR and are a pending work batch, not missing keys. A smaller set are genuinely blank ("use answer sheet") papers with no key anywhere in the archive.


## 1. No parsable answer key yet (blank papers or scanned keys pending OCR)

These written tests either have no answer key in the archived PDF (answers were on a separate answer sheet that was not saved), or carry the key only as a scanned image that still needs OCR. **63 tests across 38 competitions.**


### 2026

- **2026 Kentucky Mining Institute Mine Rescue Contest**
  - [ ] Day 1 Written Test
  - [ ] Day 2 Written Test

  > No printed key on either paper. The scan does carry a partial
  > green-pen key: 14 of the 20 questions have an ink mark on one
  > choice (detectable as green-minus-red > 25 in the page image), and
  > all 14 agree with the MSHA manual page each question cites.
  > Unmarked: Day 1 Q4, Q6, Q7, Q10 and Day 2 Q2, Q7. Held back
  > pending a real key rather than deriving those six.

- **2026 National Mine Rescue Contest**
  - [ ] Day 1 Written Test
  - [ ] Day 2 Written Test

  > Both papers carry a hand-circled key, but neither is
  > complete. Day 1 (10 questions) has circles on Q3-Q10 only;
  > Q1 and Q2 are unmarked even at 300 dpi. Day 2 is a single
  > scanned page ending at Q7 with no page number — it breaks at
  > the same point Day 1 does before continuing to a second
  > page, so Q8-Q10 are most likely a missing scan page rather
  > than a shorter test. Ask the organizer for Day 2 page 2 and
  > the two missing Day 1 answers.

### 2024

- **2024 Colorado Regional Mine Rescue Contest**
  - [ ] Written Test
- **2024 Fallen Heroes Mine Rescue Contest**
  - [ ] First Aid Written Test
- **2024 Harlan County Safety Days Competition**
  - [ ] D1 MR Written Exam
  - [ ] D2 MR Written Exam
  - [ ] First Aid Written Test
- **2024 Kentucky Mining Institute Mine Rescue Contest**
  - [ ] First Aid Written Test
- **2024 National Mine Rescue and First Aid Competition**
  - [ ] First Aid Written Test
- **2024 Nevada Underground Mine Rescue Contest**
  - [ ] BG4 Written Test
- **2024 Southeast Regional Mine Rescue Contest**
  - [ ] Day 2 Written Test

### 2023

- **2023 Colorado Regional Mine Rescue Contest**
  - [ ] Mine Rescue Day 2 Written Test
- **2023 Fallen Heroes Mine Rescue Contest**
  - [ ] Day 1 Written Test
  - [ ] Day 2 Written Test
  - [ ] Bench Written Test
- **2023 Harlan County Safety Days Competition**
  - [ ] Day 1 Written Test
  - [ ] Bench Written Test
- **2023 Southeast Regional Mine Rescue Contest**
  - [ ] Day 1 Written Test
  - [ ] Day 2 Written Test
- **2023 Virginia Governor's Cup Mine Rescue Contest**
  - [ ] Mine Rescue Written Test
  - [ ] First Aid Written Test
  - [ ] BG4 Written Test

### 2022

- **2022 67 th Southern West Virginia Mine Rescue Contest**
  - [ ] Written Test
- **2022 Fallen Heroes Mine Rescue Contest**
  - [ ] Day 1 Written Test
  - [ ] Day 2 Written Test
- **2022 Harlan County Safety Days Mine Rescue Contest**
  - [ ] Day 1 Written Test
  - [ ] Day 2 Written Test

### 2021

- **2021 Virginia Governor's Cup Mine Rescue Contest**
  - [ ] Written Test

### 2019

- **2019 NMRA Post 6 Ohio Valley Mine Rescue Contest**
  - [ ] Written Test
- **2019 Nevada Regional Mine Rescue Contest**
  - [ ] General Written Test
  - [ ] First Aid Written Test
  - [ ] Technician Team Written Test
  - [ ] Team Trainer Written Test
- **2019 Rocky Mountain Mine Rescue Contest (Craig, Colorado)**
  - [ ] Written Test
- **2019 Southern Regional Mine Rescue Contest**
  - [ ] First Aid Written Test
- **2019 Virginia Governor's Cup Mine Rescue Contest**
  - [ ] Mine Rescue Written Test
  - [ ] 240R Written Test

### 2018

- **2018 National Metal and Nonmetal Mine Rescue Contest**
  - [ ] Field Written Test

### 2017

- **2017 Southern Regional Contest**
  - [ ] Mine Rescue Written Test
  - [ ] First Aid Written Test

### 2016

- **2016 NMRA Post 6 Contest**
  - [ ] Preshift Written Test
- **2016 Tennessee Regional Mine Rescue Contest**
  - [ ] Technician Written Test

### 2015

- **2015 Harlan County Safety Days Contest**
  - [ ] First Aid Written Test
- **2015 Kentucky Mining Institute Kentucky State Mine Rescue Contest**
  - [ ] First Aid Written Test

### 2014

- **2014 Missouri Regional Mine Rescue Contest**
  - [ ] First Aid Written Test
  - [ ] Technician Drager BG4 Written Test
  - [ ] Technician BioPak 240R Written Test
  - [ ] Technician Multi-gas Detector Written Test
- **2014 Southern Regional Mine Rescue Contest**
  - [ ] Field Written Test
  - [ ] First Aid Written Test
- **2014 Southwest Regional Mine Rescue Contest**
  - [ ] Field Written Test
  - [ ] Technician Drager BG4 Written Test
- **2014 Walter Energy Mine Rescue Contest**
  - [ ] Field Written Test

### 2013

- **2013 Central Mine Rescue Contest**
  - [ ] Technician Written Test
- **2013 MTTC Pennsylvania Rescue Skills Contest**
  - [ ] Written Test
- **2013 NMRA Post 2 Mine Rescue Contest**
  - [ ] Kentucky Post 2 Bench Written Test
- **2013 Ohio Valley Mine Rescue Contest NMRA Post 6**
  - [ ] BG-4 Bench Written Test
  - [ ] BioPak 240-R Bench Written Test

### 2012

- **2012 SW Wyoming Mutual Aid Mine Rescue Contest**
  - [ ] First Aid Written Test
  - [ ] Technician Written Test
  - [ ] Team Trainer Written Test


## 2. Answer key exists but needs a manual look

These have an answer key, but the format could not be parsed into a reliable quiz (e.g. scanned images with no text, graphical answer choices like brattice symbols, or an unusual answer-marking style). **8 tests across 8 competitions.**


### 2024

- **2024 Colorado Regional Mine Rescue Contest**
  - [ ] BIO Test  _( no-parse )_

### 2023

- **2023 Colorado Regional Mine Rescue Contest**
  - [ ] First Aid Written Test  _( key/highlight:ffff00(partial) )_
- **2023 Missouri Regional Mine Rescue Contest**
  - [ ] Drager Written Test  _( no-parse )_
- **2023 West Virginia State Intra Agency Mine Rescue Contest**
  - [ ] Day 1 Written Test  _( key/highlight:ffff00(partial) )_

### 2022

- **2022 Virginia Governor's Cup Mine Rescue Contest**
  - [ ] Written Test  _( key/citation(partial) )_

### 2021

- **2021 Southwest Wyoming Mutual Aid Mine Rescue Contest**
  - [ ] Day 2 Written Test  _( separate-list/number(partial) )_

### 2014

- **2014 Alabama State Mine Rescue Contest**
  - [ ] Day 1 Written Test  _( key/textcolor:ff0000(partial) )_
- **2014 Southwest Regional Mine Rescue Contest**
  - [ ] First Aid Written Test  _( key/citation(partial) )_